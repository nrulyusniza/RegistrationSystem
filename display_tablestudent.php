<!DOCTYPE HTML>
<html>
<head>
    <title>List of all Students</title>
 
    <!-- Latest compiled and minified Bootstrap CSS -->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" />
 
    <!-- custom css -->
    <style>
    .m-r-1em{ margin-right:1em; }
    .m-b-1em{ margin-bottom:1em; }
    .m-l-1em{ margin-left:1em; }
    .mt0{ margin-top:0; }
    </style>
 
</head>
<body>
 
    <!-- container -->
    <div class="container">
 
        <div class="page-header">
            <h1>List of all Students</h1>
        </div>
 
        <!-- PHP code to read records will be here -->
 
    </div> <!-- end .container -->
 
<!-- jQuery (necessary for Bootstrap's JavaScript plugins) -->
<script src="https://code.jquery.com/jquery-3.2.1.min.js"></script>
 
<!-- Latest compiled and minified Bootstrap JavaScript -->
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
 
<!-- confirm delete record will be here -->
 
</body>
</html>
<?php
include 'connstudent.php';

$display = "<table class='table table-hover table-responsive table-bordered'>
<tr>
<th>Student ID</th>
<th>Student Name</th>
<th>Email</th>
<th>Password</th>
<th>Phone No</th>
<th>Action</th>
</tr>";

$result = mysqli_query($conn, "select * from student");
while($row = mysqli_fetch_array($result)){
    $display .= "<tr><td>$row[0]</td>";
    $display .= "<td><detailsstudent.php?studentid=$row[0]>$row[1]</a></td>";
    $display .= "<td>$row[2]</td>";
    $display .= "<td>$row[3]</td>";
    $display .= "<td>$row[4]</td>";
    $display .= "<td><a href='detailsstudent.php?studentid=$row[0]' class='btn btn-info m-r-1em'>View</a>";
    $display .= "&nbsp;<a href='updatestudent.php?studentid=$row[0]' class='btn btn-primary m-r-1em'>Update</a>";
    $display .= "&nbsp;<a href='deletestudent.php?studentid=$row[0]' class='btn btn-danger'>Delete</a></td>";
    $display .= "</tr>";
}
$display .= "</table>";
$display .= "<td><a href='indexadmin.php?' class='btn btn-primary m-r-1em'>Back to Admin</a></td>";
echo $display;
mysqli_free_result($result);
mysqli_close($conn);
?>





<?php
/*include 'connstudent.php';

$display = "<table border='1'>
<tr>
<th>Student ID</th>
<th>Student Name</th>
<th>Email</th>
<th>Phone Number</th>
<th>Action</th>
</tr>";

$result = mysqli_query($conn, "select * from student");
while($row = mysqli_fetch_array($result)){
    $display .= "<tr><td>$row[0]</td>";
    $display .= "<td><a href=detailsstudent.php?studentid=$row[0]>$row[1]</a></td>";
    $display .= "<td>$row[2]</td>";
    $display .= "<td>$row[3]</td>";
    $display .= "<td><a href=updatestudent.php?studentid=$row[0]>Update</a>";
    $display .= "&nbsp;<a href=deletestudent.php?studentid=$row[0]>Delete</a></td>";
    $display .= "</tr>";
}
$display .= "</table>";
echo $display;
mysqli_free_result($result);
mysqli_close($conn);
?>*/